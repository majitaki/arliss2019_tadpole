...
bool NavigatingState::onInit(const struct timespec& time)
{
	...
	mFarModePID = PID(NAVIGATING_UPDATE_INTERVAL_TIME, NAVIGATING_MAX_DELTA_ANGLE, -NAVIGATING_MAX_DELTA_ANGLE, 1, 0, 0);
	mCheckStuckCount = 0;
	mNearNaviCount = 0;
	mStuckTime = time;
	mFreezeTime = time;
...
void NavigatingState::navigationFarMode()
{
	...
	goalAngle = VECTOR3::calcAngleXY(currentPos, mGoalPos);//ゴールの方向
	gGPSSensor.getDirectionAngle(roverAngle);

	double deltaAngle = 0;
	deltaAngle = gAccelManager.normalize(roverAngle - goalAngle);//間の角度
