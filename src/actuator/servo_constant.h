#pragma once

const static double SERVO_UPDATE_INTERVAL_TIME = 0.1; //second
const static int ENIN_ID = 25;
//basic
const static int MAX_RAW_VALUE = 11500;
const static int CENTER_RAW_VALUE = 7500;
const static int MIN_RAW_VALUE = 3500;

//neck
<<<<<<< HEAD
const static int NECK_ID = 2;
const static int NECK_INNER = 7000;
const static int NECK_CENTER = 7500;
const static int NECK_OUTER = 9000;
=======
const static int NECK_ID = 3;
const static int NECK_INNER = 5200;
const static int NECK_CENTER = 7500;
const static int NECK_OUTER = 7500;
>>>>>>> 1e5699d738261a433e39924bd17966a082bd4e04
//direct
const static int DIRECT_ID = 0;
const static int DIRECT_RIGHT = 6000;
const static int DIRECT_CENTER = 7500;
const static int DIRECT_LEFT = 9000;
//waist
const static int WAIST_ID = 2;
const static int WAIST_INNER = 9500;
const static int WAIST_CENTER = 7500;
const static int WAIST_OUTER = 5500;
//stabi
const static int STABI_ID = 1;
const static int STABI_INNER = 5000;
const static int STABI_CENTER = 5100;
const static int STABI_OUTER = 7500;

